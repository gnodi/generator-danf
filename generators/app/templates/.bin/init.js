    ...
    Object.checkType = function(value, type, interpret, name) {
        var types = type.split('|'),
            ...
            getArticle = function(word) {
                return word[0] in {a: 1, e: 1, i: 1, o: 1} ? 'an' : 'a';
            },
        ...
        var instead = Object.getTypeString(interpretedValue, true),
            error = new Error('The expected value{0} is {1}; {2} given instead.'.format(
...
if (!Object.getTypeString) {
    /**
     * Get the type string of a value.
     *
     * @param {mixed} value The value.
     * @param {boolean|null} withValue Whether or not to retrieve the value.
     * @api public
     */
    Object.getTypeString = function(value, withValue) {
        var typeString = '',
            typeName = Object.getType(value)
        ;
        ...
        if (null !== value && 'object' === typeof value) {
            ...
            if (implementedInterfaces) {
                typeName = 'instance of [`{0}`]'.format(implementedInterfaces.join('`, `'));
            }
        }
        ...
        switch (typeName) {
            case 'undefined':
            case 'null':
                typeString = '"{0}"'.format(typeName);

                break;
            default:
                var stringValue;

                if (withValue && -1 === typeName.indexOf('instance')) {
                    stringValue = JSON.stringify(value);
                }

                typeString = '{0} "{1}"{2}'.format(
                    typeName[0] in {a: 1, e: 1, i: 1, o: 1} ? 'an' : 'a',
                    typeName,
                    null != stringValue ? ' of value `{0}`'.format(stringValue) : ''
                );
        }
...
if (!Function.prototype.defineDependency) {
    ...
    Function.prototype.defineDependency = function(property, type, providedType) {
        ...
        Object.checkType(providedType, 'string|null');
...
if (!Object.checkDependencies) {
     ...
     * @throws {error} If one dependency is missing or is of the wrong type.
     * @api public
     */
    Object.checkDependencies = function(object) {
        if (undefined === object.__metadata) {
            return;
        }
                ...
                if (providedType) {
                    if (dependency instanceof Object) {
                        if (dependency.providedType) {
                            ...
                            throw new Error(
                                'The object{0} expected a provider of "{1}" for its property "{2}"; {3} with no "providedType" getter given instead.'.format(
                                    id,
                                    providedType,
                                    property,
                                    Object.getTypeString(dependency, true)
                                )
                            );
                        }
                    } else {
                        throw new Error(
                            'The object{0} expected a provider of "{1}" for its property "{2}"; {3} given instead.'.format(
                                id,
                                providedType,
                                property,
                                Object.getTypeString(dependency, true)
...
define("node_modules/danf/lib/common/init", function(){});
